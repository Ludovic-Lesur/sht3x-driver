...
#define SHT3X_FULL_SCALE                    0xFFFF

#define SHT3X_MEASURE_COMMAND_BUFFER_SIZE   2
#define SHT3X_MEASURE_DATA_BUFFER_SIZE      6

#define SHT3X_CONVERSION_DELAY_MS           30

#define SHT3X_TEMPERATURE_SLOPE             1750
#define SHT3X_TEMPERATURE_OFFSET            (-450)

#define SHT3X_HUMIDITY_SLOPE                100
...
SHT3X_status_t SHT3X_get_temperature_humidity(uint8_t i2c_address, int32_t* temperature_tenth_degrees, int32_t* humidity_percent) {
    ...
    status = SHT3X_HW_delay_milliseconds(SHT3X_CONVERSION_DELAY_MS);
    if (status != SHT3X_SUCCESS) goto errors;
    // Read data.
    ...
    (*temperature_tenth_degrees) = (((SHT3X_TEMPERATURE_SLOPE * data_16bits) / (SHT3X_FULL_SCALE)) + SHT3X_TEMPERATURE_OFFSET);
    // Compute humidity.
    data_16bits = (int32_t) ((measure_buf[3] << 8) + measure_buf[4]);
    (*humidity_percent) = (((SHT3X_HUMIDITY_SLOPE * data_16bits)) / (SHT3X_FULL_SCALE));
errors:
    return status;
}
